...
const playAnimalName = (element) => {
  const animal = element.getAttribute('data-index');
  const nombreAnimal = document.querySelector(".AUX_nombre_animal");
  nombreAnimal.querySelector('source').setAttribute('src', `AUD/Juego2/nombres/${animal}.m4a`);
  nombreAnimal.load();
  nombreAnimal.play();
};
...
const nuevaRonda = () => {
    const randomIdx1 = Math.floor(Math.random() * animales.length);
    let randomIdx2 = Math.floor(Math.random() * animales.length);
    while (randomIdx2 === randomIdx1) {
        randomIdx2 = Math.floor(Math.random() * animales.length);
    }
    ...
    const animalesElegidos = [animales[randomIdx1], animales[randomIdx2]];

    opciones.forEach((opcion, index) => {
        const animal = animalesElegidos[index];
        opcion.setAttribute('data-index', animal);
        opcion.querySelector('h2').textContent = animal;
        opcion.style.backgroundImage = `url(IMG/Juego2/${animal}.jpg)`;
    });

    animalAudio = document.getElementById(opcionCorrecta).getAttribute('data-index');
    console.log(animalAudio+'lel')
    audioAnimal.querySelector('source').setAttribute('src', `AUD/Juego2/ruidos/${animalAudio}.mp3`);
    audioAnimal.load();
